...
function init() {
    ...
    if (raw) {
        ...
        state = { ...state, ...parsed };
        // Force the headings as requested
        state.headings = ['Personal', 'Important'];
        
        if (!state.scraps)     state.scraps     = [];
        if (!state.dailyTasks) state.dailyTasks = [];
        if (!state.history)    state.history    = {};
        
        // Migrate existing notes to the new headings if they don't match
        if (state.notes) {
            state.notes.forEach(n => {
                if (!state.headings.includes(n.heading)) {
                    n.heading = 'Personal'; // Default migration
                }
            });
        }
    }
    ...
    setInterval(() => {
        renderTimeBar();
        renderDots(); // Update today's dot color periodically
    }, 60000);
}
        ...
        state.headings.forEach(heading => {
            ...
            list.forEach((note, i) => {
                ...
                setupLongPress(row, (x, y) => {
                    // Need to find correct index in state.notes
...
window.showScrapMenu = (e, i) => {
    ...
    showContextMenu(rect.left, rect.bottom, [
        { label: 'To Personal', action: () => archiveScrap(i, 'Personal') },
        { label: 'To Important', action: () => archiveScrap(i, 'Important') }
    ]);
};
